...
>
> In staging or production deployments, `OLLAMA_BASE_URL` must be set in `infra/terraform/terraform.tfvars` or passed through your Terraform variables.
> `http://localhost:11434` and `http://127.0.0.1` are invalid in Lambda and will prevent the app from initializing the LLM backend.
>
